...
func PlanTypeAndDurationToDescription(planType string, planDuration int64) string {
	switch planType {
	case job.JobPlanTypeBasic:
		return fmt.Sprintf("Basic Option x %d months", planDuration)
	case job.JobPlanTypePro:
		return fmt.Sprintf("Pro Option x %d months", planDuration)
	case job.JobPlanTypePlatinum:
		return fmt.Sprintf("Platinum Option x %d months", planDuration)
	}
